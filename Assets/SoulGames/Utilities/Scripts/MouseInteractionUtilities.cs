...
namespace SoulGames.Utilities
{
    public class MouseInteractionUtilities
    {
        ...
        public static bool IsMousePointerOverUI() 
        {
            ...
            PointerEventData eventData = new PointerEventData(EventSystem.current) { position = GetCurrentMousePosition() };
        ...
        {
            // Priority 1: Check for VR input (NEW!)
            if (SoulGames.VR.VRMouseInterceptor.IsVRInputActive())
            {
                return SoulGames.VR.VRMouseInterceptor.GetVRMousePosition();
            }
            
            // Priority 2: Get the input position from either touch or mouse
