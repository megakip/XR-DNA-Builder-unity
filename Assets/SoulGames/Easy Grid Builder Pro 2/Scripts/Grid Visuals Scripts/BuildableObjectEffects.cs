...
namespace SoulGames.EasyGridBuilderPro
{
    ...
    public class BuildableObjectEffects : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // Check if ColorableObject exists and disable material changes if it does
            ColorableObject colorableObject = GetComponent<ColorableObject>();
            if (colorableObject != null)
            {
                changeObjectMaterial = false;
                Debug.Log($"BuildableObjectEffects: Disabled material changes on {gameObject.name} because ColorableObject is present");
            }
        ...
        {
            if (buildableObject.GetUniqueID() != this.buildableObject.GetUniqueID()) return;
            // Don't change materials if ColorableObject is present to preserve colors
            if (changeObjectMaterial && GetComponent<ColorableObject>() == null)
            {
        ...
        private void OnHoverExitByBuildableObjectSelectorDelegate(BuildableObject buildableObject)
        {
            ...
            // No color changes on hover exit - keeping current material
            // if (buildableObjectSelector != null && buildableObjectSelector.GetSelectedObjectsList().Contains(buildableObject))
            // {
        ...
        {
            if (buildableObject.GetUniqueID() != this.buildableObject.GetUniqueID()) return;
            // Don't reset materials if ColorableObject is present to preserve colors
            if (changeObjectMaterial && GetComponent<ColorableObject>() == null)
            {
                ApplyBaseMaterial();
            }
