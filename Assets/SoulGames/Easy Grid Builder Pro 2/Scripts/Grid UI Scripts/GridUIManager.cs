...
{
    /// <summary>
    /// Enum for defining UI panel positions on screen
    /// Used by GridUIManager and custom UI scripts for consistent positioning
    /// </summary>
    public enum UIPanelPosition
    {
        TopLeft,
        TopCenter, 
        TopRight,
        ...
        BottomCenter,
        BottomRight,
        Custom // Custom position allows manual override
    }
    ...
    public class GridUIManager : MonoBehaviour
    {
        ...
        public class BuildableObjectsUIPanelData
        {
            ...
            public UIPanelPosition panelPosition = UIPanelPosition.TopRight;
            public Vector2 positionOffset = new Vector2(-16, -16);
            public bool updatePositionOnStart = true;
        }
        ...
        public class SelectorUIPanelData
        {
            ...
            [Space]
            [Header("Panel Position Settings")]
            public bool followSelectedObject = true;
            public bool clampToScreenBounds = true;
            
            [Space]
            [Header("World Space Offset (VR Positioning)")]
            [Tooltip("Offset in world space relative to the selected object (X=Left/Right, Y=Up/Down, Z=Forward/Back)")]
            public Vector3 worldSpaceOffset = new Vector3(0, 1, 0);
            
            [Space]
            [Header("Additional Screen Space Offset (Only for Screen Space UI)")]
            [Tooltip("Additional offset in screen pixels - only used for non-VR Screen Space UI")]
        ...
        private void Start()
        {
            ...
            if (buildableObjectsUIPanelData.buildableObjectsUIPanel && buildableObjectsUIPanelData.updatePositionOnStart)
            {
                UpdateBuildableObjectsUIPanelPosition();
            }
        }

        private void Update()
        {
            // Continuously update selector panel position if it's following a selected object
            if (isSelectorUIPanelEnabled && selectorUIPanelData.followSelectedObject && 
                buildableObjectSelector && buildableObjectSelector.GetSelectedObjectsList().Count > 0)
            {
                UpdateSelectorUIPanelPosition();
            }
        }
        ...
        private IEnumerator OnSelectedByBuildableObjectSelectorDelegate()
        {
            ...
            // Position the selector panel above the selected object
            if (selectorUIPanelData.followSelectedObject)
            {
                UpdateSelectorUIPanelPosition();
            }
        }
        ...
        private IEnumerator InvokeOnDeselectedByBuildableObjectSelectorDelegate()
        {
            ...
            if (displaySelectorPanelOnObjectSelection)
            {
                if (buildableObjectSelector.GetSelectedObjectsList().Count == 0)
                {
                    isSelectorUIPanelEnabled = false;
                    FadeOut(selectorUIPanelData.selectorUIPanel.GetComponent<CanvasGroup>());
                }
                else
                {
                    UpdateSelectorUIPortraitImage();
        ...
        {
            if (!displaySelectorPanelOnObjectSelection) return;
            BuildableObjectSO buildableObjectSO = buildableObjectSelector.GetSelectedObjectsList()[buildableObjectSelector.GetSelectedObjectsList().Count - 1].GetBuildableObjectSO();
            selectorUIPanelData.selectorPortraitImage.sprite = buildableObjectSO.objectIcon;
        }
        ...
        {
            StartFade(canvasGroup, canvasGroup.alpha, 0);
        }
        ...
        /// <summary>
        /// Updates the position of the Buildable Objects UI Panel based on the configured settings
        /// </summary>
        private void UpdateBuildableObjectsUIPanelPosition()
        {
            if (!buildableObjectsUIPanelData.buildableObjectsUIPanel) return;
            
            RectTransform panelRect = buildableObjectsUIPanelData.buildableObjectsUIPanel;
            Vector2 anchorMin, anchorMax, pivot;
            Vector2 anchoredPosition = buildableObjectsUIPanelData.positionOffset;
            
            switch (buildableObjectsUIPanelData.panelPosition)
            {
                ...
                case UIPanelPosition.BottomRight:
                    ...
                    break;
                case UIPanelPosition.Custom:
                    // Don't change anchors/pivot for custom positioning
                    return;
                default:
                    return;
            }
        ...
        /// <summary>
        /// Updates the position of the Selector UI Panel to appear relative to the selected object using world space offset
        /// </summary>
        private void UpdateSelectorUIPanelPosition()
        {
            if (!selectorUIPanelData.selectorUIPanel || !buildableObjectSelector) return;
            
            var selectedObjects = buildableObjectSelector.GetSelectedObjectsList();
            if (selectedObjects.Count == 0) return;
            
            // Get the last selected object (the most recent selection)
            BuildableObject selectedObject = selectedObjects[selectedObjects.Count - 1];
            if (!selectedObject) return;
            
            UpdateSelectorUIPanelPositionForObject(selectedObject.gameObject);
        }
        
        /// <summary>
        /// Updates the position of the Selector UI Panel to appear relative to a specific GameObject using world space offset
        /// For VR World Space UI - directly sets the world position of the UI panel
        /// </summary>
        /// <param name="targetObject">The GameObject to position the panel relative to</param>
        private void UpdateSelectorUIPanelPositionForObject(GameObject targetObject)
        {
            if (!selectorUIPanelData.selectorUIPanel || !targetObject) return;
            
            // Check if this is a World Space Canvas (VR UI)
            Canvas parentCanvas = selectorUIPanelData.selectorUIPanel.GetComponentInParent<Canvas>();
            if (parentCanvas && parentCanvas.renderMode == RenderMode.WorldSpace)
            {
                // VR World Space UI - directly set world position
                Vector3 worldPosition = targetObject.transform.position + selectorUIPanelData.worldSpaceOffset;
                
                // Set the panel's world position directly
                selectorUIPanelData.selectorUIPanel.position = worldPosition;
                
                Debug.Log($"GridUIManager: VR UI Panel positioned at world position: {worldPosition} (Target: {targetObject.name}, Offset: {selectorUIPanelData.worldSpaceOffset})");
            }
            else
            {
                // Screen Space UI - use original screen space conversion method
                // Calculate world position with world space offset
                Vector3 worldPosition = targetObject.transform.position + selectorUIPanelData.worldSpaceOffset;
                
                // Convert world position to screen position
                Vector3 screenPosition = SoulGames.Utilities.MouseInteractionUtilities.GetWorldToScreenPosition(worldPosition);
                
                // Apply additional screen space offset
                screenPosition += new Vector3(selectorUIPanelData.screenSpaceOffset.x, selectorUIPanelData.screenSpaceOffset.y, 0);
                
                // Convert screen position to canvas local position
                RectTransform canvasRect = parentCanvas.GetComponent<RectTransform>();
                Vector2 localPosition;
                
                if (RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRect, screenPosition, parentCanvas.worldCamera, out localPosition))
                {
                    // Clamp to screen bounds if enabled
                    if (selectorUIPanelData.clampToScreenBounds)
                    {
                        RectTransform panelRect = selectorUIPanelData.selectorUIPanel;
                        Vector2 panelSize = panelRect.sizeDelta;
                        
                        // Get canvas size
                        Vector2 canvasSize = canvasRect.sizeDelta;
                        
                        // Clamp within canvas bounds accounting for panel size
                        localPosition.x = Mathf.Clamp(localPosition.x, -canvasSize.x * 0.5f + panelSize.x * 0.5f, canvasSize.x * 0.5f - panelSize.x * 0.5f);
                        localPosition.y = Mathf.Clamp(localPosition.y, -canvasSize.y * 0.5f + panelSize.y * 0.5f, canvasSize.y * 0.5f - panelSize.y * 0.5f);
                    }
                    
                    // Set the panel's anchored position
                    selectorUIPanelData.selectorUIPanel.anchoredPosition = localPosition;
                }
            }
        }
        
        /// <summary>
        /// Public function to show the selector panel for a specific GameObject
        /// This can be called from external scripts like SimpleSeeInsideUI
        /// </summary>
        /// <param name="targetObject">The GameObject to show the selector panel for</param>
        public void ShowSelectorPanelForObject(GameObject targetObject)
        {
            if (!targetObject || !selectorUIPanelData.selectorUIPanel) return;
            
            // Enable the panel if it's not already enabled
            if (!isSelectorUIPanelEnabled)
            {
                isSelectorUIPanelEnabled = true;
                if (selectorUIPanelData.selectorUIPanel.TryGetComponent<CanvasGroup>(out CanvasGroup canvasGroup))
                {
            ...
            UpdateSelectorUIPanelPositionForObject(targetObject);
            
            // Update portrait image if the object has a BuildableObject component
            BuildableObject buildableObject = targetObject.GetComponent<BuildableObject>();
            if (buildableObject && selectorUIPanelData.selectorPortraitImage)
            {
        ...
        /// <summary>
        /// Public function to hide the selector panel
        /// </summary>
        public void HideSelectorPanel()
        {
            if (isSelectorUIPanelEnabled && selectorUIPanelData.selectorUIPanel)
            {
                isSelectorUIPanelEnabled = false;
                if (selectorUIPanelData.selectorUIPanel.TryGetComponent<CanvasGroup>(out CanvasGroup canvasGroup))
                {
        ...
        /// <summary>
        /// Test function to check if VR UI positioning is working correctly
        /// Call this from the Inspector or another script to test
        /// </summary>
        [ContextMenu("Test VR UI Panel Positioning")]
        public void TestVRUIPanelPositioning()
        {
            if (!selectorUIPanelData.selectorUIPanel)
            {
                Debug.LogError("GridUIManager: Geen selector UI panel gevonden!");
                return;
            }
            ...
            if (!parentCanvas)
            {
                Debug.LogError("GridUIManager: Geen canvas gevonden!");
                return;
            }
            
            Debug.Log($"GridUIManager: Canvas Render Mode: {parentCanvas.renderMode}");
            Debug.Log($"GridUIManager: UI Panel Current Position: {selectorUIPanelData.selectorUIPanel.position}");
            Debug.Log($"GridUIManager: World Space Offset: {selectorUIPanelData.worldSpaceOffset}");
            
            // Test positioning at origin + offset
            Vector3 testPosition = Vector3.zero + selectorUIPanelData.worldSpaceOffset;
            selectorUIPanelData.selectorUIPanel.position = testPosition;
            Debug.Log($"GridUIManager: Test position set to: {testPosition}");
        }
    }
}