...
namespace SoulGames.EasyGridBuilderPro
{
    ...
    public class EasyGridBuilderProXZ : EasyGridBuilderPro
    {
        ...
        protected override void UpdateAutoDetectAndSetVerticalGrid()
        {
            ...
            Vector3 surfaceWorldPosition = SoulGames.Utilities.MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(autoDetectionLayerMask, customSurfaceLayerMask | gridManager.GetGridSystemLayerMask(),
        ...
        bool isBuildableEdgeObjectFlipped = false, int activeVerticalGridIndex = -1, bool invokedByFinalizedBoxPlacement = false, bool forceFreeObjectRotation = false)
        {
            ...
            if (!isSceneObject)
            {
                // Check if mouse pointer is on grid - this might need adjustment for VR later if it causes issues
                // For now, let MouseInteractionUtilities handle the ray source.
                // The original check was: if (!gridManager.GetIsMousePointerOnGrid() || gridManager.GetActiveEasyGridBuilderPro() != this) return;
                // We'll rely on the raycast from MouseInteractionUtilities to determine valid targets.
                // A more robust check here would be if the ray from GetInputRay() hits the grid collider.
            ...
            {
                // MODIFIED SECTION: Use MouseInteractionUtilities
                if (buildableObjectSO is BuildableFreeObjectSO freeSO) // Cast to use in MouseInteractionUtilities call
                {
                    worldPosition = SoulGames.Utilities.MouseInteractionUtilities.GetMouseWorldPositionForBuildableFreeObject(
                        freeSO.customSurfaceLayerMask | gridManager.GetGridSystemLayerMask(), 
                        freeSO, 
                        Vector3.down * 99999, // Original fallback direction
                        out hitNormals
                    );
                }
                else
                {
                     // Assuming a similar structure to GetMouseWorldPositionWithCustomSurface
                     // Need to ensure the out parameters match if MouseInteractionUtilities has them.
                     // The original call was:
                     // worldPosition = SoulGames.Utilities.MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(activeBuildableObjectSO.customSurfaceLayerMask, gridManager.GetGridSystemLayerMask(), Vector3.down * 9999,
                     //    out bool _, out Vector3 firstCollisionWorldPosition);
                     // worldPosition.y = firstCollisionWorldPosition.y;

                    // Using a simpler GetMouseWorldPosition from MouseInteractionUtilities for non-free objects if the specific 'WithCustomSurface' variant isn't perfectly matched.
                    // Or, if MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface exists and matches, use that.
                    // For this example, let's assume GetMouseWorldPositionWithCustomSurface is available and similar in MouseInteractionUtilities:
                    worldPosition = SoulGames.Utilities.MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(
                        activeBuildableObjectSO.customSurfaceLayerMask,
                        gridManager.GetGridSystemLayerMask(),
                        Vector3.down * 9999, // Original fallback direction
                        out bool directHitGrid, // We might not use this directly here but the signature needs to match
                        out Vector3 firstCollisionWorldPos // We might not use this directly here but the signature needs to match
                    );
                    // If the VR utility doesn't handle the y-adjustment or second ray internally in the same way,
                    // that logic might need to be replicated here or added to the VR utility.
                    // For now, we assume worldPosition returned by MouseInteractionUtilities is the correct one.
                }
        ...
        {
            // MODIFIED: Use MouseInteractionUtilities
            Vector3 worldPosition = SoulGames.Utilities.MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(
                activeBuildableObjectSO.customSurfaceLayerMask, 
                gridManager.GetGridSystemLayerMask(), 
                Vector3.down * 9999f, // Original fallback direction
                out bool directHitGridCollisionLayer, 
                out Vector3 firstCollisionWorldPosition 
            );

            // IMPORTANT: The original EGB code had specific logic here, often:
            // worldPosition.y = firstCollisionWorldPosition.y;
            // Or logic dependent on 'directHitGridCollisionLayer'.
            // Ensure your MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface 
            // returns the final, correct worldPosition that EGB expects, or replicate that logic here.
            // For now, this returns the direct result from your utility.
        ...
        {
            // MODIFIED: Use MouseInteractionUtilities
            return SoulGames.Utilities.MouseInteractionUtilities.GetMouseWorldPositionForBuildableFreeObject(
                activeBuildableObjectSO.customSurfaceLayerMask | gridManager.GetGridSystemLayerMask(), 
                (BuildableFreeObjectSO)activeBuildableObjectSO, 
                Vector3.down * 99999f, // Original fallback direction
                out hitNormals
                // The original call might have had a maxDistance parameter. 
                // Ensure your MouseInteractionUtilities.GetMouseWorldPositionForBuildableFreeObject handles this if necessary.
            );
        }
        ...
        private void UpdateGridObjectBoxPlacement()
        {
            ...
            boxPlacementEndPosition = SoulGames.Utilities.MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(activeBuildableObjectSO.customSurfaceLayerMask, layerMask, Vector3.down * 9999,
