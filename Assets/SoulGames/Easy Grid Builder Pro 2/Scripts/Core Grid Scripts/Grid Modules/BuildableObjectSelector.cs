    ...
    public class BuildableObjectSelector : MonoBehaviour
    {
        ...
        public void SetInputSelectionMove()
        {
            ...
            if (gridManager.TryGetBuildableObjectMover(out BuildableObjectMover buildableObjectMover))
            {
                ...
                // Don't reset selection for move - keep selector UI panel open
                // ResetAreaSelection();
                // ResetIndividualSelection();
            }
        }
        ...
        {
            // Don't reset selection for rotate/flip operations - keep selector UI panel open
            // Clear the old selection list without firing deselection events
            selectedObjectsList.Clear();
            
            // Set the new selection list and fire selection events
        ...
        private void RotateSelectedBuildableGridObject(BuildableObjectDestroyer buildableObjectDestroyer, BuildableGridObject buildableGridObject, bool rotateClockwise, ref List<BuildableObject> newSelectedObjectList)
        {
            ...
            // FIXED: Use the exact transform position instead of calculating from cell position
            Vector3 objectOriginWorldPosition = buildableGridObject.transform.position;
            ...
            Debug.Log($"[RotationFix] Rotating object at position: {objectOriginWorldPosition}, VerticalIndex: {verticalGridIndex}");
            ...
            while (initializationSuccessful == false && iterationCount < 4)
            {
                ...
                if (easyGridBuilderPro.TryInitializeBuildableGridObjectSinglePlacement(objectOriginWorldPosition, buildableGridObjectSO, nextFourDirectionalRotation, true, true, verticalGridIndex, true,
                    out newBuildableGridObject, objectRandomPrefab, buildableGridObject)) 
                {
                    initializationSuccessful = true;
                    
                    // ADDITIONAL FIX: Force the exact position after creation
                    if (newBuildableGridObject != null)
                    {
                        newBuildableGridObject.transform.position = objectOriginWorldPosition;
                        Debug.Log($"[RotationFix] New object placed at: {newBuildableGridObject.transform.position}");
                    }
                }
        ...
        private void RotateSelectedBuildableEdgeObject(BuildableObjectDestroyer buildableObjectDestroyer, BuildableEdgeObject buildableEdgeObject, bool rotateClockwise, ref List<BuildableObject> newSelectedObjectList)
        {
            ...
            // FIXED: Use the exact transform position instead of GetObjectModifiedOriginWorldPosition
            Vector3 objectOriginWorldPosition = buildableEdgeObject.transform.position;
            ...
            while (initializationSuccessful == false && iterationCount < 4)
            {
                ...
                if (easyGridBuilderPro.TryInitializeBuildableEdgeObjectSinglePlacement(objectOriginWorldPosition, buildableEdgeObjectSO, nextFourDirectionalRotation, isObjectFlipped, true, true, 
                    verticalGridIndex, true, out newBuildableEdgeObject, objectRandomPrefab, buildableEdgeObject)) 
                {
                    initializationSuccessful = true;
                    ...
                    {
                        newBuildableEdgeObject.transform.position = objectOriginWorldPosition;
                    }
                }
        ...
        private void FlipBuildableEdgeObject(BuildableEdgeObject buildableEdgeObject, BuildableObjectDestroyer buildableObjectDestroyer, ref List<BuildableObject> newSelectedObjectList)
        {
            ...
            // FIXED: Use the exact transform position
            Vector3 objectOriginWorldPosition = buildableEdgeObject.transform.position;
        ...
        public void SetInputSelectionCopy()
        {
            ...
            // Don't reset selection for copy - keep selector UI panel open
            // ResetAreaSelection();
            // ResetIndividualSelection();
        }
        ...
        private void InvokeSelectBuildableObject()
        {
            ...
            if (SoulGames.Utilities.MouseInteractionUtilities.IsMousePointerOverUI()) return;
        ...
        public List<BuildableObject> GetSelectedObjectsList()
        {
